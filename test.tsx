...
const TestNine = (props: { callback: ({$foo}: { $foo: string }) => void, foo: string }) => {
    const {
        callback,
        foo
    } = props
    return <button type='button' onClick={() => callback({$foo: 'bar'})}>{foo}</button>
}
...
const TestAngularNine = react2angular(TestNine, {
    callback: '&',
    foo: '<'
})
